...
import com.inka.ncg2.LicenseConfig;
...
//			mNcg2Agent.setHttpRequestCallback(mHttpRequestCallback );
	...
	public void acquireLicense(Activity activity, String contentPath, String token) {
		ProgressDialog progress = ProgressDialog.show(activity, "Wait", "Please wait...", true);
		boolean isTemporaryLicense = false;
		if (URLUtil.isHttpsUrl(contentPath) || URLUtil.isHttpUrl(contentPath)) {
			isTemporaryLicense = true;
		}
		mNcg2Agent.acquireLicense(
				new LicenseConfig.Builder(token, isTemporaryLicense)
						.acquisitionURL("https://license.pallycon.com/ri/ncg/licenseManager.do")
						.build(),
	...
	public void updateSecureTime() {
		mNcg2Agent.updateSecureTime(
				s -> mListener.onCompletedUpdateSecureTime(),
				e -> mListener.onError(e, "Failed to update SecureTime")
		);
	}

	public void updateSecureTimeAsyncTask() {
